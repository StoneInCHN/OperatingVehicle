				    	...
				    	<tr>
				    		...
				    		<td>
			    			  <select id="isEnabled" class="easyui-combobox" name="isEnabled" style="width:45px;" data-options="panelMaxHeight:55">   
								<option value="true" [#if systemConfig.isEnabled ==true] selected="selected" [/#if]>${message("ov.common.yes")}</option>
								<option value="false" [#if systemConfig.isEnabled ==false] selected="selected" [/#if]>${message("ov.common.no")}</option> 
							  </select>  
	    					</td>
				    	</tr>
			    </table>
