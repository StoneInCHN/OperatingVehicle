...
<head>
<title>${message("ov.role.list")}</title>
...
<div class="mainbar">
				<div class="page-head">
					<div class="bread-crumb">
						<a ><i class="fa fa-male"></i>${message("ov.main.role")}</a> 
						<span class="divider">/</span> 
						<span  class="bread-current"><i class="fa fa-list"></i>${message("ov.role.list")}(${message("ov.page.total", page.total)})</span>
					</div>
						  		...
						  		<div class="input-group">
								      <div class="input-group-btn">
								        ...
								        <ul class="dropdown-menu menuWrap" id="searchPropertyOption" role="menu">
								          <li selected="selected" class="active" title="name"><a href="#">${message("ov.role.name")}</a></li>
								        </ul>
								      </div>
			              ...
			              <div class="col-md-12">
			                <div class="widget">
									 <div class="widget-head">
						                  <div class="pull-left"><i class="fa fa-list"></i>${message("ov.main.role")}</div>
										...
										<table id="listTable" class="table table-striped table-bordered table-hover">
											<thead>
												<tr>
													...
													<th>
														<a href="javascript:;" class="sort" name="name">${message("ov.role.name")}</a>
													</th>
													<th>
														<a href="javascript:;" class="sort" name="isSystem">${message("ov.role.isSystem")}</a>
													</th>
													<th>
														<span>${message("ov.role.description")}</span>
													</th>
													<th>
											...
											<tbody>
												[#list page.content as role]
													<tr>
														<td>
															<input type="checkbox" name="ids"[#if role.isSystem] title="${message("ov.role.deleteSystemNotAllowed")}" disabled="disabled"[#else] value="${role.id}"[/#if] />
														</td>
														<td>
															${role.name}
														</td>
														<td>
															${message(role.isSystem?string('ov.common.true', 'ov.common.false'))}
														</td>
														<td>
															[#if role.description??]
														...
														<td>
															<a href="edit.jhtml?id=${role.id}" title="${message("ov.common.edit")}"><i class="fa fa-pencil-square-o"></i></a>
														</td>
													</tr>
												[/#list]
											</tbody>
